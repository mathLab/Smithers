...
class TestOpenFoamHandler(TestCase):
    ...
    def test_read_cell_neighbors(self):
        ...
        smithers_cell = mesh['0']['cells'][a_key]
        np.testing.assert_almost_equal(
            smithers_cell['neighbours'],
            truth_mesh.cell_neighbour[a_key])
    ...
    def test_read_cell_points(self):
        ...
        faces_points = np.concatenate([truth_mesh.faces[face_idx] for face_idx in faces_idxes])
        faces_points = np.unique(faces_points)

        np.testing.assert_almost_equal(
            smithers_cell['points'],
            faces_points
        )
    ...
    def test_read_fields_time_instants_list(self):
        handler = OpenFoamHandler()
        time_list_mesh = handler.read(openfoam_mesh_path, fields_time_instants=['1088'])
