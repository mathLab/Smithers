...
class TestNetAdapter(TestCase):
    ...
    def test_reducenet_03(self):
        ...
        red_net = netadapter.reduce_net(seq_model, train_dat, tgts, train_load, 4)
    ...
    def test_reducenet_04(self):
        ...
        red_net = netadapter.reduce_net(seq_model, train_dat, tgts, train_load, 14)
        input_ = torch.arange(1 * 3 * 224 * 224,
                              dtype=torch.float32).view(1, 3, 224, 224)
        out = red_net(input_)
        self.assertEqual(list(out.size()), [1, 14])
