...
class Testutils(TestCase):
    def test_get_seq_model_01(self):
        ...
        self.assertEqual(len(seq_model), len(model.features) + len(model.classifier) + 2)
    ...
    def test_constructor_spatial_gradients_01(self):
        ...
        input = [(torch.rand(3, 224, 224), torch.tensor([5]))]
        grad = spatial_gradients(input, pre_model, post_model)
    ...
    def test_constructor_spatial_gradients_02(self):
        ...
        input1 = (torch.rand(3, 224, 224), torch.tensor([5]))
        input2 = (torch.rand(3, 224, 224), torch.tensor([9]))
        input3 = (torch.rand(3, 224, 224), torch.tensor([3]))
    ...
    def test_spatial_gradients_01(self):
        ...
        gradients = []
        for grad in spatial_gradients(input, pre_model, post_model):
            gradients.append(grad)
    ...
    def test_spatial_gradients_02(self):
        ...
        input = [input1, input2, input3]
        gradients = []
        generator = spatial_gradients(input, pre_model, post_model)
        for i in range(2):
            grad = next(generator)
            gradients.append(grad)
    ...
    def test_forwarddataset_01(self):
        inps = torch.arange(10 * 3 * 224 * 224, dtype=torch.float32).view(10, 3, 224, 224)
        tgts = torch.arange(10, dtype=torch.float32)#.view(10, 5)
        dataset = TensorDataset(inps, tgts)
        model = torch.hub.load('pytorch/vision:v0.10.0', 'vgg16', pretrained=True)
        model.classifier_str = 'standard'
        data_loader = DataLoader(dataset, batch_size=2, pin_memory=True)
